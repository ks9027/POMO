...
class CVRPEnv: #환경을 설정하고 데이터를 관리하며 상태를 추적하고 업데이트하는 역할을 하는 클래스
    ...
    def _get_travel_distance(self):
        gathering_index = self.selected_node_list[:, :, :, None].expand(-1, -1, -1, 2) # 마지막에 2인 이유는 (x,y)의 노드 인덱스를 가져오기 위함임
        # shape: (batch, pomo, selected_list_length, 2)
        all_xy = self.depot_node_xy[:, None, :, :].expand(-1, self.pomo_size, -1, -1) #각 배치와 pomo인덱스에 대해 모든 노드의 좌표를 반복하여 확장한 텐서
        # shape: (batch, pomo, problem+1, 2)

        ordered_seq = all_xy.gather(dim=2, index=gathering_index) #gathering_index를 사용하여 all xy 텐서에서 각 노드의 좌표를 추출하며 이 코드는 선택된 노드들의 좌표를 순서대로 나열하겠다는 의미
        # shape: (batch, pomo, selected_list_length, 2)

        rolled_seq = ordered_seq.roll(dims=2, shifts=-1) #ordered_seq을 한 칸씩 왼쪽으로 이동시킨 텐서로 마지막 노드는 처음으로 이동
        segment_lengths = ((ordered_seq-rolled_seq)**2).sum(3).sqrt() #ordered_seq과 rolled_seq사이의 차이를 구하고 이를 제곱한 후 마지막 차원에서 제곱하여 마지막 차원에서 합산하여 각 구간의 거리의 제곱을 계산
